...
const Buttons = ({ formData }) => {
    ...
    const generatePDF = async () => {
        try {
            ...
            // ===== CAPTURAR LA PARTE SUPERIOR COMO IMAGEN =====
            const formElement = document.querySelector(".quality-inspection-form")

            if (formElement) {
                // Obtener los elementos de la parte superior
                const headerElements = formElement.querySelectorAll(":scope > div:nth-child(-n+3)")

                if (headerElements.length > 0) {
                    // Crear un contenedor temporal para capturar los elementos juntos
                    ...
                    // Añadir la imagen de la parte superior al PDF
                    pdf.addImage(headerImgData, "JPEG", margin, y, headerImgWidth, headerImgHeight)
                    y += headerImgHeight + 5
                } else {
                    // Si no se pueden capturar los elementos, crear un encabezado básico
                    pdf.setFontSize(16)
            ...
            // Añadir una línea horizontal después de la parte superior
            y = addHorizontalLine(y)
            y += 5

            // ===== CUESTIONARIO =====
            y = checkPageBreak(y, 40) // Verificar si hay espacio para el cuestionario
            y = addSectionHeader("CUESTIONARIO", y)
            ...
            const questionRows = questions.map((q, index) => {
                ...
                const value = formData.questionnaire[questionKey]
                return [q, value === "C" ? "X" : "", value === "NC" ? "X" : ""]
            })

            const questionColWidths = [contentWidth * 0.8, contentWidth * 0.1, contentWidth * 0.1]
            // Usar un multiplicador de altura de fila más alto para el cuestionario
            ...
            // Ahora esta sección va después del cuestionario
            y = checkPageBreak(y, 20)
            y = addSectionHeader("INFORMACIÓN DE INSPECCIÓN", y)
            ...
            const criticalRows = criticalDefectsList.map((defect, index) => [
                defect,
                formData.criticalDefects[index]?.units || "-",
            ])
            ...
            const majorRows = majorDefectsList.map((defect, index) => [defect, formData.majorDefects[index]?.units || "-"])
            ...
            const minorRows = minorDefectsList.map((defect, index) => [defect, formData.minorDefects[index]?.units || "-"])
            ...
            if (formData.signatures.assistant || formData.signatures.chief) {
                ...
                if (formData.signatures.assistant) {
                    try {
                        const img = new Image()
                        img.src = formData.signatures.assistant
                        await new Promise((resolve) => {
                            img.onload = resolve
                            setTimeout(resolve, 1000) // Timeout por si la imagen no carga
                        })
                ...
                if (formData.signatures.chief) {
                    try {
                        const img = new Image()
                        img.src = formData.signatures.chief
                        await new Promise((resolve) => {
                            img.onload = resolve
                            setTimeout(resolve, 1000) // Timeout por si la imagen no carga
                        })
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            alert("Error generar PDF: " + error.message);
        } finally {
    ...
    const handleSaveToDrive = async () => {
        ...
        try {
            ...
            if (!saved) throw new Error("Could not save form locally");
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            if (!saved) throw new Error("Could not save email record locally");
